...
def _ensure_columns():
    ...
    try:
        ...
        # Non-fatal: tables may already be fully migrated
        import logging
        logging.warning("_ensure_columns: %s", exc)
...
def list_follow_requests(
    ...
    """Pending requests that others sent TO me."""
...
def use_invite(
    ...
    The caller also follows the owner automatically.
    """
            ...
            # Server fans out an alert to everyone in the same conversation.
                ...
                try:
                    ...
                    if ctx_type == "dm" and isinstance(ctx_id, int):
                        ...
                        room = db_ws.query(Room).filter_by(token=ctx_id).first()
                        if room:
